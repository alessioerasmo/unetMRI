...
dir_img_val = Path('./data/validation_imgs/')
dir_mask_val = Path('./data/validation_masks/')
...
def train_model(
    ...
    # scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=5, verbose=True)  # goal: maximize Dice score (SCHEDULER ORIGINALE)
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[5,10,20, 40, 60, 80], gamma=0.1) # consigliato dal prof
    ...
    for epoch in range(1, epochs + 1):
        scheduler.step() # if milestones scheduler, consigliato dal prof
        model.train()
        epoch_loss = 0
            ...
            for batch in train_loader:
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        # scheduler.step(val_score) # STANDARD if reduce LR on plateau scheduler

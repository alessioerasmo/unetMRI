...
name_model = 'runs/Tracked runs/100epoche_restricted_3enc_MultistepLR_crisp.pth'
...
np_masks = np.array(masks, dtype='float64')
np_result = np.array(ress, dtype='float64')
#np_result = np.array(hardening(ress), dtype='float64')
...
nii_ress = nib.Nifti1Image(np_result, affine=GULEaffine)

#nib.save(nii_mask, "ConsensusROGU.nii.gz")
nib.save(nii_ress, "BinaryClassifierGULE.nii.gz")
